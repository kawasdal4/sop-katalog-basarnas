...
// Get Excel file buffer from R2
async function getExcelFileBuffer(document: { id: string; filePath: string; fileName: string }): Promise<{ buffer: Buffer; error?: string }> {
  try {
    if (!document.filePath) {
      return { buffer: Buffer.alloc(0), error: 'File tidak tersedia di storage' }
    }

    // Check if R2 is configured
    if (!isR2Configured()) {
      return { buffer: Buffer.alloc(0), error: 'Cloudflare R2 tidak terkonfigurasi' }
    }

    console.log(`📥 Downloading Excel from R2: ${document.filePath}`)
    
    const result = await downloadFromR2(document.filePath)
    
    console.log(`✅ Excel buffer from R2: ${result.buffer.length} bytes`)
  ...
  try {
    ...
    if (excelResult.error || excelResult.buffer.length === 0) {
      ...
      return NextResponse.json({ 
        ...
        suggestion: 'Pastikan file tersedia atau hubungi admin.'
      }, { status: 404 })
    }
